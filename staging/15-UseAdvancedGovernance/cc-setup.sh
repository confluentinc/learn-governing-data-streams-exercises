...
SR_KEY=$(cat creds/$CLUSTER_NAME-sr-key.json | jq -r '.key')
SR_SECRET=$(cat creds/$CLUSTER_NAME-sr-key.json | jq -r '.secret')
...
echo "CREATING TOPIC: OrderCreated"
confluent kafka topic create OrderCreated
confluent sr schema create --schema ./src/main/resources/avro/OrderCreated-value.avsc --subject OrderCreated-value --api-key "$SR_KEY" --api-secret "$SR_SECRET"

echo "CREATING TOPIC: PaymentSucceeded"
confluent kafka topic create PaymentSucceeded
confluent sr schema create --schema ./src/main/resources/avro/PaymentSucceeded-value.avsc --subject PaymentSucceeded-value --api-key "$SR_KEY" --api-secret "$SR_SECRET"
